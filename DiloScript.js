...
function konfirmasiKembali(targetPage, message) {
    bukaKonfirmasi(message, "Ya, Kembali", function() { nav(targetPage); });
}
...
function tampilkanPanduan(halaman) {
    ...
    switch(halaman) {
        ...
        case 'materi':
            ...
            pesan = "<ul style='text-align:left; margin:0; padding-left:20px;'><li>Silakan pilih <b>Unit Gerak Lokomotor</b> dan <b>Permainan</b>.</li><li style='margin-top:10px;'>Alur berjalan lurus: <b>Pilih Materi ➔ Instruksi & Video ➔ Absensi ➔ Penilaian</b>.</li></ul>";
            showModal(judul, pesan, true);
            break;
        case 'absen':
        ...
        case 'penilaian':
            ...
            pesan = "<ul style='text-align:left; margin:0; padding-left:20px;'><li>Gunakan tombol umpan balik di bawah nama siswa untuk melakukan penilaian.</li><li>Sebuah form akan muncul, masukan nilai siswa dan catatan (opsional), lalu tekan simpan.</li><li style='margin-top:10px;'>Tombol nilai <b>tetap aktif</b> untuk memudahkan perbaikan atau debugging jika Anda ingin mengubah nilai pada sesi yang sama.</li></ul>";
            showModal(judul, pesan, true);
            break;
        case 'evaluasi':
...
function downloadTemplateSiswa() {
    ...
    XLSX.writeFile(wb, "Template_Import_Siswa_Dilomotors.xlsx");
}
...
function initDataGuru() {
    try {
        const saved = localStorage.getItem('dilo-guru');
        if(saved) {
...
function simpanDataGuru() {
    ...
    localStorage.setItem('dilo-guru', JSON.stringify(dataGuru));
...
function renderBreadcrumbs() {
    ...
    pages.forEach(page => {
        ...
        bc.innerHTML = `<div class="guru-nama">${dataGuru.nama}</div><div class="guru-nip">${textNIP}</div>`;
...
function renderTabelSiswa() {
    ...
    dataSiswa.forEach(siswa => {
        tbody.innerHTML += `<tr>
            <td style="font-weight:900; color:#8E0000;">${siswa.absen}</td>
...
function bukaFormTambahSiswa() {
    ...
    const autoSekolah = dataGuru && dataGuru.sekolah ? dataGuru.sekolah : (sessionStorage.getItem('tempSekolahDilo') || '');
...
function simpanSiswa() {
    ...
    if(id) {
        ...
        if(index !== -1) dataSiswa[index] = { id_siswa: id, absen: absen, nama_siswa: nama, nisn: nisn, jk: jk, kelas: kelas, sekolah: sekolah };
    } else {
...
function prosesImportExcel(event) {
    ...
    reader.onload = function(e) {
        ...
        if(jumlahBerhasil > 0) {
            ...
            showModal("Peringatan", "Tidak ada data yang valid dimasukkan. Pastikan kolom 'Nama' dan 'Kelas' tersedia.");
        }
        event.target.value = '';
    };
...
function pilihSubMateri(id, index){
    ...
    document.getElementById('teksTujuan').innerText = tmpTujuan.length <= 0 ? `Tujuan pembelajaran dari permainan adalah melakukan latihan gerak lokomotor "${materi.nama_materi}."` : tmpTujuan;
    ...
    if(materi.permainan[index].video.length >=1){
        materi.permainan[index].video.forEach((v) => {
            htmlVideo += `
        ...
        htmlVideo += `
        <div class="content-box">
            <p style="font-weight:900; color:#C62828; margin-top:0;">🎥 Animasi dalam produksi.</p>
            <div style="height:auto; background:#FDECEA; color:#C62828; font-weight:800; display:flex; align-items:center; justify-content:center; border-radius:15px; border:2px dashed #F8D7DA;">
...
function renderAbsensi() {
    ...
    if(dataSiswa.length <= 0){
        container.innerHTML = `<div class="illustration-area"><img class="login-icon" src="assets/empty.png" alt="aktor-siswa-kosong"></div><p class="kembali" style="font-size:1.2em; color:#fff;">Data siswa tidak ditemukan</p><button class="btn-warning" onclick="nav('page-data-siswa')">Isi Data Siswa</button>`;
        return;
    }
...
function renderPenilaian() {
    ...
    siswaKelasIni.forEach(siswa => {
       ...
       /* if (!isHadir) {
            ...
            if (isDinilai) statusText = `<br><span style="color:#4CAF50; font-size:13px;">✔ Nilai Masuk Ke Riwayat</span>`;
            
            // Tombol tetap aktif agar dapat dilakukan evaluasi berulang
            actionButtons = `
        ...
        if (!isHadir) {
            ...
            // === TAMBAHKAN LOGIKA KUNCI DI SINI ===
            if (isDinilai) {
...
function renderTabelEvaluasi() {
    ...
    dataSiswa.forEach(siswa => {
        ...
        // Berdasarkan ID Lokomotor (L1 - L6)
        const avgL1 = getAvgMateri(siswa.id_siswa, 'L1'); 
        const avgL2 = getAvgMateri(siswa.id_siswa, 'L2'); 
        const avgL3 = getAvgMateri(siswa.id_siswa, 'L3'); 
        const avgL4 = getAvgMateri(siswa.id_siswa, 'L4'); 
        const avgL5 = getAvgMateri(siswa.id_siswa, 'L5'); 
        const avgL6 = getAvgMateri(siswa.id_siswa, 'L6'); 
        ...
        tbody.innerHTML += `<tr>
            ...
            <td>${siswa.nisn || '-'}</td>
            <td>${siswa.kelas}</td>
            <td style="font-weight:900; color:#C62828; background:#FFEBEE;">${avgTotal}</td>
            <td>${avgL1}</td><td>${avgL2}</td><td>${avgL3}</td>
            <td>${avgL4}</td><td>${avgL5}</td><td>${avgL6}</td>
...
function exportExcel() {
    ...
    excelData.push([`Evaluasi Hasil Penilaian Aplikasi Dilomotors ${namaSekolah}`.trim()]);
    excelData.push([`Nama Guru : ${namaGuru}`]);
    excelData.push([`NIP       : ${nipGuru}`]);
    excelData.push([textKelas]);
    excelData.push([]); 
    
    excelData.push(["No Abs", "Nama Siswa", "NISN", "Kelas", "Rata-rata Total", "Running", "Galloping", "Hopping", "Jumping", "Skipping", "Sliding"]);
    ...
    siswaDiproses.forEach(siswa => {
        ...
        excelData.push([
            ...
            getAvgMateri(siswa.id_siswa, 'L1'), getAvgMateri(siswa.id_siswa, 'L2'),
            getAvgMateri(siswa.id_siswa, 'L3'), getAvgMateri(siswa.id_siswa, 'L4'),
            getAvgMateri(siswa.id_siswa, 'L5'), getAvgMateri(siswa.id_siswa, 'L6')
        ]);
    });
    ...
    ws['!cols'] = [ {wpx: 40}, {wpx: 160}, {wpx: 80}, {wpx: 60}, {wpx: 100}, {wpx: 70}, {wpx: 70}, {wpx: 70}, {wpx: 70}, {wpx: 70}, {wpx: 70} ];
    
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Rekap Evaluasi");
    XLSX.writeFile(wb, `Evaluasi_${kelasFilter ? kelasFilter : 'SemuaKelas'}_${namaSekolah ? namaSekolah.replace(/\s+/g, '_') : 'Dilomotors'}.xlsx`);
}
