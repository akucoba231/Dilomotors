    ...
    function pilihMateri(id) {
        ...
        materi.video.forEach((v, i) => {
            htmlVideo += `<div class="content-box"><p style="font-weight:900; color:#8E24AA; margin-top:0;">🎥 Video ${i+1}: ${v.judul}</p><div style="height:140px; background:#F3E5F5; color:#BA68C8; font-weight:800; display:flex; align-items:center; justify-content:center; border-radius:15px; border:2px dashed #E1BEE7;">[Player Video]</div></div>`;
        });
    ...
    function renderAbsensi() {
        ...
        dataSiswa.forEach(siswa => {
            ...
            // Pengecekan status untuk menonaktifkan tombol (Disable logic)
            const isDiabsen = tempAbsen[siswa.id_siswa] ? 'disabled' : '';
            
            container.innerHTML += `<div class="content-box">
                <p style="font-weight:900; margin-top:0; color:#8E24AA; font-size:16px;">${siswa.nama_siswa} <span style="font-size:13px; color:#9E9E9E;">(${status})</span></p>
    ...
    function simpanAlasanAbsen() { 
        ...
        renderAbsensi(); // Merender ulang untuk menerapkan 'disabled'
    }
    ...
    function renderPenilaian() {
        ...
        dataSiswa.forEach(siswa => {
            ...
            // Pengecekan jika sudah ada nilai, matikan tombolnya (Disable logic)
            const isDinilai = hasScore ? 'disabled' : '';

            container.innerHTML += `<div class="content-box">
                <p style="font-weight:900; margin-top:0; color:#8E24AA; font-size:15px;">${siswa.nama_siswa} <span style="font-size:12px; color:#9E9E9E;">(${tempAbsen[siswa.id_siswa] || 'Hadir'})</span> ${statusNilai}</p>
    ...
    */
    ...
    function simpanNilaiSiswa() {
        ...
        renderPenilaian(); // Merender ulang agar tombol langsung terkunci (disabled)
    }
    ...
    function exportExcel() {
        ...
        link.setAttribute("download", "Evaluasi_PJOK_Dilomotors.csv"); document.body.appendChild(link);
